...
def utility_panel(pubs: list[dict], path: Path) -> None:
    ...
    ax.set_title("Best feasible hardware utility versus greedy incumbent and exact optimum")
...
def ghz_panel(ghz: dict, path: Path) -> None:
    ...
    axes[0].set_title("GHZ Hellinger fidelity")
    axes[0].set_xlabel("Run")
    axes[0].legend(["sampled Aer baseline", "exact GHZ baseline"], frameon=False)
    ...
    for ax in axes:
        _style(ax)
    fig.suptitle("Run 20 is the long queue outlier. Log axis is labelled.", color=NAVY, fontsize=10)
    fig.tight_layout()
    fig.savefig(path.with_suffix(".svg"), bbox_inches="tight")
    fig.savefig(path.with_suffix(".png"), dpi=180, bbox_inches="tight")
    plt.close(fig)
...
def architecture_diagram(path: Path) -> None:
    ...
    ax.set_title("Observed hardware path (top) versus post-collection replay (bottom right).", color=NAVY)
    fig.tight_layout()
    fig.savefig(path.with_suffix(".svg"), bbox_inches="tight")
    fig.savefig(path.with_suffix(".png"), dpi=180, bbox_inches="tight")
    plt.close(fig)
