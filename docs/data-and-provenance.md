...
| `post_collection_policy_replay` | `results/publication/tables/replay_adapter_*.json` | Current 1,920-row adapter on archived shots |
| `superseded_2026_09_09_replay` | `archive/superseded-analysis/2026-09-09-post-collection-replay/` | Older replay path; not current adapter evidence |
| `sampled_simulation` | derived local sampling | Not hardware |

The current `decision-study/reports/CHATGPT_RETURN_REPORT.md` is a **historical** mixed document (completed-run totals together with setup-era unpublished/future statements). Do not treat it as the current public report. The dated current report is `results/publication/reports/CURRENT_REPORT.md` after a successful `--refresh-publication` (analysis generated UTC is recorded separately from the 2026-09-09 collection date), or `build/reproduction/reports/RESULTS.md` from isolated reproduction.
...
`decisions.json` (~5 MB) and `policy_replay.json` (~8 MB) are retained frozen decision-study products. Current publication tables are those regenerated by `tools/reproduce.py`. See `results/publication/SOURCE_TO_OUTPUT_INDEX.json` after refresh.
